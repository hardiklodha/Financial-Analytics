...
denco2 = read.csv("./data/denco.csv")
...
str(sales)
dim(sales)      #dimensions of df rows and columns
...
df3a= aggregate(sales$margin, by=list(sales$partnum), FUN=sum)
head(df3a)
df3a[order(-df3a$x),][1:5,]

df3b = aggregate(margin ~ partnum, data=sales, FUN=sum)
head(df3b)
